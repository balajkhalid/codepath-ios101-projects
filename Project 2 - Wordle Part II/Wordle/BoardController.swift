...
class BoardController: NSObject,
  ...
  private func applyThemeSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      guard let wordThemeString = settings[kWordThemeKey] as? String else{
          assertionFailure("Expecting String, but got nil")
          return
      }
      guard let wordTheme = WordTheme(rawValue: wordThemeString) else{
          assertionFailure("Expecting wordTheme, but got nil")
          return
      }
  ...
  private func applyIsAlienWordleSettings(with settings: [String: Any]) {
    // START YOUR CODE HERE
      guard let isAlienWordleSettings = settings[kIsAlienWordleKey] as? Bool else{
          assertionFailure("Expecting Bool, but got nil")
          return
      }
      isAlienWordle = Bool(isAlienWordleSettings)
    // END YOUR CODE HERE
  }
}
